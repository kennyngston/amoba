...
void create(object obj) {
  ...
  ob=obj;
  set_name("Piros X");
  set_id(({"x","piros","iksz","jel","kereszt"}));
  set_short("%^BOLD%^%^RED%^Piros X%^RESET%^");
  set_long("Egy apró piros %^BOLD%^%^RED%^X%^RESET%^. Egy manótól kaptad a %^BOLD%^%^WHITE%^szabályok%^RESET%^kal egyetemben, ezzel tudsz %^BOLD%^%^WHITE%^rak%^RESET%^ni, illetve %^BOLD%^%^WHITE%^felad%^RESET%^ni a játékot.\n");
  set_min_level(1);
  set_weight(0);
  set_value(0);
